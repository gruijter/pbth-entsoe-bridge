...
1. Create a new R2 bucket in your Cloudflare dashboard named entsoe-prices.
2. Go to the bucket settings and connect your custom domain (e.g., entsoe-prices.gruijter.org).

### 2. Edge Caching Setup
To allow Cloudflare to cache the dynamic JSON files and serve millions of requests for free:
1. Go to your domain settings in Cloudflare -> Caching -> Cache Rules.
2. Create a rule matching: Hostname equals entsoe-prices.gruijter.org.
3. Set Cache Eligibility to Eligible for cache.
4. Set Edge TTL to Respect origin header. (The Worker automatically sends a 5-minute cache header for prices, and a 1-minute header for the status file).
